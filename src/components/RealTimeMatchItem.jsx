    ...
    if (layout === 'mobile') {
        return (
            ...
            >
                ...
                {(match.is_live || match.is_critical_moment) && (
                    <div className="absolute top-2 right-2 flex items-center space-x-1">
                        <div className={`w-2 h-2 rounded-full animate-pulse ${match.is_critical_moment ? 'bg-red-600' : 'bg-red-500'}`}></div>
                        <span className={`text-xs font-semibold ${match.is_critical_moment ? 'text-red-700' : 'text-red-600'}`}>
                            {match.is_critical_moment ? 'CRITICAL' : 'LIVE'}
                        </span>
                    </div>
                )}
