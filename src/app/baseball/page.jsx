...
export default function BaseballPage() {
  ...
  const fetchGames = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/baseball`);
  ...
  const handleToggleFavorite = (gameId) => {
    setFavorites(prev => {
      ...
      localStorage.setItem('baseball_favorites', JSON.stringify(newFavorites));
      return newFavorites;
    });
  };
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-5">
              {/* Stats Header */}
              <div className="bg-gradient-to-r from-red-600 to-red-500 rounded-xl p-4 mb-4 text-white">
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center">
                    <span className="text-2xl">⚾</span>
                  </div>
                  <div>
                    <h1 className="font-bold text-lg font-condensed">Baseball</h1>
                    <p className="text-red-100 text-sm font-condensed">
                      MLB • NPB • KBO • Liga Domestik
                    </p>
                  </div>
                </div>
            ...
            <div className="hidden lg:block lg:col-span-3">
              ...
              <div className="bg-white rounded-xl shadow-sm p-4 mt-4">
                <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-lg p-4 text-white text-center">
                  ...
                  <p className="font-bold font-condensed">Baseball Live</p>
                  <p className="text-sm text-red-100 font-condensed">Nonton MLB & NPB!</p>
                  <button className="mt-3 px-4 py-2 bg-white text-red-600 rounded-lg text-sm font-bold font-condensed hover:bg-red-50 transition-colors">
                    STREAMING
                  </button>
                </div>
              </div>
